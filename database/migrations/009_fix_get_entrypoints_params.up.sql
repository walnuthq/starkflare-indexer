...
DROP FUNCTION IF EXISTS starkflare_api.get_entrypoints_table(VARCHAR(66));

CREATE OR REPLACE FUNCTION starkflare_api.get_entrypoints_table(contract_address VARCHAR(66))
RETURNS TABLE (
...
BEGIN
    ...
    WHERE starkflare_api.account_calls.contract_address = contract_address
      AND timestamp >= last_7_days_timestamp;
...
CREATE OR REPLACE FUNCTION starkflare_api.get_entrypoints(contract_address VARCHAR(66))
RETURNS JSON
AS $$
DECLARE
    entrypoints JSON;
BEGIN
    ...
    FROM starkflare_api.get_entrypoints_table(contract_address) AS entrypoints_table;
